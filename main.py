...
TOKEN = "8315603096:AAGo9OSSZ1GTToWMsYBS04tLH5tL4_9ww4c"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "✅ ابدأ":
        keyboard = [["👨‍🏫 مدرب"], ["👨‍🎓 متدرب"]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text("يرجى اختيار المستخدم:",
                                        reply_markup=reply_markup)
        return

    if text == "👨‍🏫 مدرب":
        keyboard = [["تظلم المدرب"], ["وصف المقررات"], ["المراجع التدريبية"],
                    ["🔙 عودة"]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text(
    ...
    if text == "🔙 عودة":
        # Check if the user was in the "awaiting_id" state
        if context.user_data.get("awaiting_id"):
            ...
            reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
            await update.message.reply_text("خدمات المتدربين:",
                                            reply_markup=reply_markup)
            return
    ...
    if text == "المراجع التدريبية":
        ...
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text("اختر القسم التدريبي المستهدف:",
                                        reply_markup=reply_markup)
        return
    ...
    if text == "📚 الدراسات العامة":
        await update.message.reply_text(
            "📚 المراجع التدريبية لقسم الدراسات العامة:\n\n"
            "يمكنك الوصول للحقائب التدريبية من خلال الرابط التالي:\n"
            "https://tvtc.gov.sa/ar/Departments/tvtcdepartments/cdd/Pages/packages.aspx?RootFolder=/ar/Departments/tvtcdepartments/cdd/DocLib1/%D8%A7%D9%84%D9%85%D8%B9%D8%A7%D9%87%D8%AF+%D8%A7%D9%84%D8%B5%D9%86%D8%A7%D8%B9%D9%8A%D8%A9/%D8%A7%D9%84%D9%85%D9%82%D8%B1%D8%B1%D8%A7%D8%AA+%D8%A7%D9%84%D8%B9%D8%A7%D9%85%D8%A9"
        )
        return

    if text == "❄️ التبريد والتكييف":
        await update.message.reply_text(
            "❄️ المراجع التدريبية لقسم التبريد والتكييف:\n\n"
            "يمكنك الوصول للحقائب التدريبية من خلال الرابط التالي:\n"
    ...
    if text == "🔍 معرفة رقمي التدريبي":
        ...
        await update.message.reply_text(
            "🔢 من فضلك أرسل رقم الهوية لمعرفة رقمك التدريبي:")
        return
    ...
    if context.user_data.get("awaiting_id"):
        ...
        try:
            ...
            with open("data/students.csv", mode="r", encoding="utf-8") as f:
                ...
                for row in reader:
                    if row["رقم الهوية"].strip() == id_number:
                        student_id = row["رقم المتدرب"]
            ...
            await update.message.reply_text(
                "⚠️ عذراً، حدث خطأ أثناء البحث في البيانات. يرجى المحاولة لاحقاً."
            )
            return
    ...
    if text == "🚗 ميكانيكا السيارات":
        ...
        await update.message.reply_text(
            f"📘 سيتم إضافة المراجع الخاصة بقسم ({text}) قريباً.")
    ...
    elif context.user_data.get("complaint_state"):
